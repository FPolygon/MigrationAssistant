...
public class FileLogProviderTests : IDisposable
{
    ...
    public async Task WriteLogAsync_ExceedsMaxFileSize_ShouldRotateFile()
    {
        ...
        settings.ProviderSettings["IncludeTimestamp"] = true; // Enable timestamp to ensure unique filenames
        _provider.Configure(settings);
        ...
        for (int i = 0; i < 100; i++)
        {
            ...
            // Add small delay to ensure different timestamps for rotation
            if (i % 20 == 0)
            {
                await Task.Delay(10);
            }
    ...
    private LoggingSettings CreateFileSettings(string directory)
    {
        ...
        var testName = System.Runtime.CompilerServices.RuntimeHelpers.GetHashCode(this).ToString("X");
